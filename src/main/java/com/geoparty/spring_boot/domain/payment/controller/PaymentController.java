...
public class PaymentController {
    ...
    public ResponseEntity<String> readyPay(
        ...
        String webURL = pointService.readyKakao(details, point);
        return ResponseEntity.status(HttpStatus.OK).body(webURL);
    }

    @PostMapping("/complete")
    @Operation(description = "카카오 페이 결제 진행.")
    public ResponseEntity<KakaopayApproveResponse> makeSave(@AuthenticationPrincipal final PrincipalDetails details, @RequestParam("pg_token") String pgToken, @RequestParam("tid")
    String tid) {
